...
@Component({
  selector: 'animation-vapp-rename',
  template: `
	  <demo label="VApp Rename" height="180"
          description="Animate vApp label size based on new name. VApp background has no change."></demo>
  ` })
export class AnimationVappRenameComponent implements AfterViewInit {
  ...
  ngAfterViewInit() {
    ...
    vapps.forEach((vappData, index) => {
      const position = new paper.Point(
        ...
        VIEW_PADDING - 6 + Math.ceil(CONNECTOR_SIZE / 2));
